...
### Быстрые ссылки
  ...
  * 🐛 [Сообщить об ошибке / проблеме](https://github.com/Kotin-ak/VRC-Releases/issues)

**VRC — это централизованное Windows-приложение, разработанное для удаленного МОНИТОРИНГА и УПРАВЛЕНИЯ несколькими экземплярами vMix с единой панели.** Хватит переключаться между разными компьютерами для управления трансляцией. С VRC у вас есть полное удаленное управление вашими нодами vMix:
  ...
  * ⏰ **Автоматизация:** Встроенный планировщик задач для автоматизации команд vMix на основе точного времени.
...
## Почему VRC? (Какую проблему это решает)
...
> [\!IMPORTANT]
> Поскольку VRC является локально подписанным пакетом .msix, вы **ОБЯЗАНЫ** сначала установить сертификат безопасности, иначе Windows заблокирует установку.

1.  Скачайте архив `.zip` со страницы Релизов (Releases) и распакуйте его.
...
**Что такое Пресеты и Конфигурации?**
  ...
  * **Пресет (Preset):** Сохраненная группа машин vMix (ваша конкретная рабочая область). Если вы перемещаетесь между разными площадками, вы можете сохранить пресет для "Студии А" и другой для "Выездного турнира". Загрузите пресет, и VRC мгновенно подключится к точным нодам vMix для этой локации, не требуя повторного ввода IP-адресов вручную.
  * **Конфигурация (Configuration):** Мастер-резервная копия вашего приложения VRC, включающая *все* ваши пресеты и глобальные настройки. Используйте это для быстрого клонирования всей вашей настройки VRC на дополнительный или резервный управляющий ноутбук.

| Действие | Сочетание клавиш | Бизнес-ценность / Описание |
|--------|----------|------------------------------|
| **Добавить vMix** | `Ctrl+N` | Подключить новую ноду vMix к текущей рабочей области |
| **Последняя сессия** | — | Быстро восстановить те машины, которые вы мониторили при последнем закрытии VRC |
| **Сохранить пресет** | `Ctrl+S` | Сохранить текущую сетку машин vMix для мгновенной загрузки в следующий раз |
| **Размер карточки** | — | Настроить, сколько места на экране занимает каждая карточка устройства |
...
| Действие | Сочетание клавиш | Бизнес-ценность / Описание |
|--------|----------|------------------------------|
| **Сохранить как…** | `Ctrl+Shift+S` | Дублировать текущую рабочую область под новым именем (например, "Турнир День 2") |
| **Удалить пресет** | — | Удалить настройку рабочей области, которая больше не нужна |
| **Экспорт пресета** | — | Сохранить конкретный макет рабочей области в файл, чтобы поделиться с другим оператором |
| **Импорт пресета** | — | Загрузить макет рабочей области, предоставленный кем-то другим |
| **Экспорт конфигурации** | — | Создать мастер-резервную копию всей настройки VRC для легкого переноса на другой ПК |
| **Импорт конфигурации** | — | Восстановить вашу мастер-резервную копию на новом управляющем ноутбуке |
...
При добавлении нового устройства vMix указываются следующие поля:
  ...
  - **Имя** — пользовательское имя (до 20 символов).
  - **IP-адрес** — адрес машины, на которой запущен vMix.
  ...
  - **Логин и пароль** — учетные данные для авторизации (если требуется).
  - **Режим транспорта** — метод связи с vMix (HTTP, TCP и т.д.). При выборе HTTP отображается предупреждение об ограничениях.
  - **Часовой пояс** — назначение часового пояса для устройства, чтобы обеспечить правильное отображение времени при удаленной работе.
...
### Параметры подключения
  ...
  - **Автоподключение** — автоматическое подключение к устройству при запуске приложения.
  - **Автопереподключение** — автоматическое восстановление соединения при его потере.

### Действия с устройством

Доступны через контекстное меню карточки:

  - **Настройки трансляции (Streaming Settings)** — открыть диалог управления каналами трансляции.
  - **Редактировать (Edit)** — изменить параметры подключения.
  - **Настройки WMI** — настроить удаленный мониторинг ПК.
  - **Журналы (Logs)** — просмотреть журнал событий устройства.
  - **Удалить (Delete)** — удалить устройство из конфигурации.
  - **Переместить в… (Move to…)** — переместить устройство между группами.
...
|-----------|-------|---------|
| **Streaming (Трансляция)** | Старт/стоп всех каналов | Индивидуальные индикаторы каналов 1–5 (каждый кликабелен). Количество каналов зависит от редакции vMix |
| **Recording (Запись)** | Старт/стоп записи | Индикаторы основного и вторичного рекордера, таймер продолжительности записи |
| **Multicorder (Мультикамера)** | Старт/стоп мультикамеры | Отображается только если поддерживается редакцией vMix |
| **Replay (Повтор)** | Старт/стоп записи Instant Replay | Отображается только если поддерживается редакцией vMix |
| **External (Внешний выход)** | Вкл/выкл внешний выход | Всплывающая подсказка с деталями конфигурации выхода |
| **Fullscreen (Полный экран)** | Вкл/выкл полноэкранный режим | Всплывающая подсказка с текущими настройками |
| **Playlist (Плейлист)** | Старт/стоп плейлиста | — |
| **Overlay (Оверлей)** | Отключить все оверлеи | Индивидуальные индикаторы для каждого слоя (каждый кликабелен отдельно) |
...
#### Информационная панель
  ...
  - **Имя текущего входа** — имя и метка воспроизводимого источника.
  - **Полоса прогресса** — для воспроизводимых источников (видео), показывает оставшееся время.
  - **Статус воспроизведения** — иконки Play / Pause / Stop.
  - **Loop (Зацикливание)** — индикатор зацикливания.
  - **Позиция в списке** — отображение индекса элемента для видеосписков.
  - **Текст титров** — текущий текст для входов титров.
...
| **AutoPause** (Вкл/Выкл) | Автопауза при переключении с этого входа |
| **AutoPlay** (Вкл/Выкл) | Автовоспроизведение при переключении на этот вход |
| **AutoRestart** (Вкл/Выкл) | Автоперезапуск по завершении |
...
| **Индикатор звука** | Двухканальный индикатор уровня L/R |
...
#### Список элементов
  ...
  - **Контекстное меню**: Выбрать (Select), Удалить (Remove - убрать из списка).
...
Для каждого выхода отображается следующее:
  ...
  - **Source (Источник)** — текущий назначенный источник (можно изменить для Output 2–4).
...
| Элемент | Описание |
|---------|-------------|
| **🔒 Lock (Замок)** | Блокировка. Состояние по умолчанию: ВКЛ. Защищает от случайных действий. Когда активно, все кнопки управления заблокированы. Кликните для переключения. |
| **🔽 Свернуть аудио** | Показать/скрыть раздел аудио (с анимацией вращения иконки) |
| **🔔 Уведомления** | Включить/отключить уведомления для конкретного устройства (зеленый — вкл, серый — выкл) |
| **⚠ Ошибки** | Отображение текущих ошибок подключения (красным цветом) |
| **⏳ Спиннер** | Индикатор загрузки во время выполнения операции |
| **🔘 Переключатель подключения** | Включить/отключить подключение к устройству |

-----

## 4\. Мониторинг состояния ПК (PC Health)

Удаленный сбор метрик рабочей станции для машины, на которой запущен vMix:

  - **CPU** — загрузка процессора (с подсветкой критических значений).
  - **GPU 3D** — загрузка видеокарты (3D рендеринг).
  - **GPU Encode** — загрузка аппаратного видеокодера.

Данные собираются через **WMI** (Windows Management Instrumentation). Настройки WMI доступны из меню карточки. Если удаленный мониторинг недоступен, отображается информация об ошибке WMI/RPC.
...
### Создание задачи
  ...
  - **Name (Имя)** — пользовательское имя задачи.
  - **Device (Устройство)** — выбор целевого устройства.
  - **Category and Function (Категория и Функция)** — выбор команды из иерархического каталога.
  - **Parameters (Параметры)** — дополнительные параметры команды (динамически зависят от выбранной функции).
  - **Schedule (Расписание)** — тип: одноразовая / ежедневно / еженедельно.
  - **Time and Date (Время и Дата)** — точный момент выполнения (24-часовой формат).
  - **Retries (Повторы)** — количество попыток повторения при сбое (1–10).
  - **Enable/Disable (Вкл/Выкл)** — индивидуально для каждой задачи.
...
| Действие | Описание |
|--------|-------------|
| **Hold (Удержать)** | Приостановить выбранные задачи |
| **Run Now (Запустить сейчас)** | Немедленно выполнить выбранные задачи |
| **+5 / +10 / +15 min** | Отложить выполнение на указанное время |
| **Cancel (Отменить)** | Отменить выбранные задачи |
| **Restore (Восстановить)** | Восстановить отмененные задачи |
...
#### Мастер-панель (Слева — Список шорткатов)
  ...
  - Колонки таблицы: чекбокс (вкл/выкл), индикатор статуса, Ключ (Key), Устройство (Device), Функция (Function), Заголовок (Title).
...
| Поле | Описание |
|-------|-------------|
| **Event (Событие)** | Тип события ACTS для обратной связи (например, Recording, Streaming, Input, Overlay и т.д.) |
...
| Категория | События |
|----------|--------|
| **Глобальные (Global)** | Recording, Streaming, External, Fullscreen, FadeToBlack, MasterAudio, звук шин Bus A–G |
| **Привязанные к входу (Input-bound)** | Input (Program), InputPreview, InputPlaying, InputAudio, InputSolo, InputLoop, маршрутизация входа на шины (A–G, Master), InputAudioAuto |
| **Оверлеи (Overlays)** | Overlay 1–8 |
...
### Процесс выполнения шортката (Execution Flow)
...
| Параметр | Описание |
|-----------|-------------|
| **Language (Язык)** | Выбор языка интерфейса (Русский / Английский) |
| **Minimize to Tray (Сворачивать в трей)** | Приложение сворачивается в системный трей вместо панели задач |
| **Auto-Start (Автозапуск)** | Автоматический запуск приложения при входе в Windows |
| **Multiple Instances (Несколько окон)** | Разрешить запуск нескольких копий приложения |
...
| **Mode (Режим)** | Все (All) / Критические (Critical) / Эфир (Broadcast) / Выкл (Off) |
| **Connection Notifications** | Оповещения о подключении/отключении устройства |
| **Silent Mode (Без звука)** | Подавить звуки уведомлений |

### Веб-дашборд (Web Dashboard)

| Параметр | Описание |
|-----------|-------------|
| **Enable (Включить)** | Активировать встроенный веб-сервер |
| **Port (Порт)** | Настройка порта (1–65535) |
...
| Параметр | Описание |
|-----------|-------------|
| **Version (Версия)** | Текущая версия приложения |
| **Updates (Обновления)** | Проверить, скачать и установить обновления с отображением прогресса |
...
## 10\. Навигация
  ...
  - Быстрый доступ к **Настройкам (Settings)** через встроенную кнопку навигации.
  - Отдельная страница **Планировщика (Scheduler)** для глобального управления всеми задачами.
  - Отдельная страница **Сочетаний клавиш (Shortcuts)** для привязок кнопок внешних контроллеров.
